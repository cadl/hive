 ...
 */
public class HiveHBaseTableMapredInputFormat extends TableInputFormatBase
    implements InputFormat<ImmutableBytesWritable, ResultWritable> {

  static final Logger LOG = LoggerFactory.getLogger(HiveHBaseTableMapredInputFormat.class);
  private static final Object hbaseTableMonitor = new Object();
  private Connection conn = null;

  @Override public org.apache.hadoop.mapreduce.RecordReader<ImmutableBytesWritable, Result> createRecordReader(
    ...
    final org.apache.hadoop.mapreduce.RecordReader<ImmutableBytesWritable, Result> recordReader =
        super.createRecordReader(tableSplit, context);
    ...
    return new org.apache.hadoop.mapreduce.RecordReader<ImmutableBytesWritable, Result>() {
  ...
  public InputSplit[] getSplits(JobConf jobConf, int numSplits) throws IOException {
    synchronized (hbaseTableMonitor) {
      return getSplitsInternal(jobConf);
    }
  }

  @Override public List<org.apache.hadoop.mapreduce.InputSplit> getSplits(JobContext context)
      throws IOException {
    synchronized (hbaseTableMonitor) {
      List<org.apache.hadoop.mapreduce.InputSplit> result = new ArrayList<>();
      for (InputSplit inputSplit : getSplitsInternal(new JobConf(context.getConfiguration()))) {
        result.add((org.apache.hadoop.mapreduce.InputSplit) inputSplit);
      }
      return result;
    }
  }

  private InputSplit[] getSplitsInternal(JobConf jobConf) throws IOException {
